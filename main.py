...
# ==========================================
# 1. MODUL PROXY AUTOMATION
# ==========================================
def check_local_ip_is_vn():
    print("[INIT] 🔎 Memeriksa IP asal...")
    try:
        ...
        print(f"   -> ⚠️ Lỗi check IP: {e}. Menggunakan Proxy.")
        return False
...
def find_best_proxy(proxy_list):
    ...
    for p in proxy_list:
        ...
        if ping is not None:
            ...
            if ping < 2.0:
                print(f"   ⚡ Proxy cepat ditemukan: {p['ip']}")
                return p['ip'], p['protocol']
    return best_ip, best_proto

# ==========================================
# 2. MODUL CAPTURE MASTER TOKEN VTV
# ==========================================
