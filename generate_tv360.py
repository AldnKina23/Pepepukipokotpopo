...
def grab_m3u8_token(page, target_url):
    ...
    try:
        ...
        # Pemicu otomatis (simulasi klik tombol play / video element)
        ...
        page.evaluate("""
            () => {
                ...
                const playBtn = document.querySelector('.vjs-big-play-button') || document.querySelector('.play-icon');
                if (playBtn) playBtn.click();
            }
        """)

        # Tunggu sampai request token terdeteksi
        for _ in range(15):
            if token_url:
                break
        ...
        logger.warning(f"⚠️ Timeout/Error memuat halaman: {e}")
...
def main():
    ...
    if results:
        ...
        logger.error("❌ Semua channel gagal karena geoblock server GitHub. Memuat fallback playlist kosong...")
        # Membuat file playlist kosong agar GitHub Actions tidak memicu 'Exit Code 1' Error
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        filepath = os.path.join(OUTPUT_DIR, OUTPUT_FILE)
        with open(filepath, "w", encoding="utf-8") as f:
            f.write('#EXTM3U\n# Stream TV360 terblokir lokasi IP Server GitHub Actions.\n')
